...
QK_D = 32
BLOCK_BYTES_Q3_D = 16  # [d:fp16(2)][m:fp16(2)][ql:8][qh:4]
BLOCK_BYTES_Q4_D = 20  # [d:fp16(2)][m:fp16(2)][qs:16]
BLOCK_BYTES_Q5_D = 24  # [d:fp16(2)][m:fp16(2)][qh:4][qs:16]
BLOCK_BYTES_Q6_D = 28  # [d:fp16(2)][m:fp16(2)][ql:16][qh:8]
...
def _prepare_common(
    ...
    if groupsize <= 0:
        raise ValueError("groupsize must be > 0")
    if groupsize % QK_D != 0:
        raise ValueError(f"groupsize must be a multiple of {QK_D}")
    if q_int.ndim != 2:
    ...
    num_groups = math.ceil(in_features / groupsize)
    ...
    group_ids = torch.div(torch.arange(in_features, device=scale.device), groupsize, rounding_mode="floor")
    scale_cols = scale[:, group_ids].to(torch.float32)
    zero_cols = zero[:, group_ids].to(torch.float32)
    m_cols = -zero_cols * scale_cols

    n_blocks = in_features // QK_D
    d_blocks = scale_cols[:, ::QK_D].to(torch.float16).contiguous()
    m_blocks = m_cols[:, ::QK_D].to(torch.float16).contiguous()
...
def pack_q4_d(
    ...
    """
    Pack GPTQ tensors into Q4_D/Q4_1-compatible raw block bytes.
    ...
    qs = ((q[:, :, :16] & 0x0F) | ((q[:, :, 16:] & 0x0F) << 4)).to(torch.uint8).contiguous()
...
def pack_q3_d(
    ...
    ql = _pack_bits(q & 0x03, bits_per_value=2).contiguous()          # (..., 8)
    qh = _pack_bits((q >> 2) & 0x01, bits_per_value=1).contiguous()   # (..., 4)
...
def pack_q5_d(
    ...
    qs = ((q[:, :, :16] & 0x0F) | ((q[:, :, 16:] & 0x0F) << 4)).to(torch.uint8).contiguous()
    qh = _pack_bits((q >> 4) & 0x01, bits_per_value=1).contiguous()   # (..., 4)
...
def pack_q6_d(
    ...
    ql = ((q[:, :, :16] & 0x0F) | ((q[:, :, 16:] & 0x0F) << 4)).to(torch.uint8).contiguous()  # (..., 16)
    qh = _pack_bits((q >> 4) & 0x03, bits_per_value=2).contiguous()                             # (..., 8)
    packed_tensor = _pack_block_rows(d_blocks, m_blocks, ql, qh)
    meta = {
        "out_features": out_features,
        "in_features": in_features,
